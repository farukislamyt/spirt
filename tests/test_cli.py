...
def make_result() -> CollectionResult:
    return CollectionResult(CollectionStatus.SUCCESS, SocialProfile(
        platform="facebook", profile_url="https://www.facebook.com/example", username="example",
...
def test_profile_json_includes_evidence() -> None:
    with patch("spirt.cli.get_provider") as get_provider:
        get_provider.return_value.collect.return_value = make_result()
